...
TEST(SlamBackend, NormalizesLegacyModeAliases)
{
  const auto rtabmap_alias = sb::start_backend("rtabmap_vio", "occupancy", "/map", "/rtabmap/cloud_map");
  ASSERT_TRUE(rtabmap_alias.status.ready) << rtabmap_alias.status.message;
  EXPECT_EQ(rtabmap_alias.activation.contract.mode, "vio");

  const auto liosam_alias = sb::start_backend("lio_sam", "occupancy", "/map", "/rtabmap/cloud_map");
  EXPECT_FALSE(liosam_alias.status.ready);
  EXPECT_EQ(liosam_alias.status.code, sb::StartupErrorCode::kNotImplemented);
}
