...
def _nearest_truth_sample(
    ...
    if index <= 0:
        candidate = truth_samples[0]
    elif index >= len(truth_samples):
        candidate = truth_samples[-1]
    else:
    ...
    if abs(candidate[0] - target_stamp) > max_time_offset_sec:
        return None
    return candidate
...
def _compute_drift_metrics(
    ...
    *,
    ...
    sorted_truth_times = [sample[0] for sample in sorted_truth]
    unmatched_count = 0
    for vio_stamp, vio_x, vio_y in sorted(vio_samples, key=lambda sample: sample[0]):
        truth_sample = _nearest_truth_sample(
            sorted_truth,
            sorted_truth_times,
            vio_stamp,
            max_time_offset_sec,
        ...
        if truth_sample is None:
            unmatched_count += 1
            continue
        truth_stamp, truth_x, truth_y = truth_sample
        error_m = math.hypot(vio_x - truth_x, vio_y - truth_y)
        matched_rows.append(
            {
                "stamp_sec": vio_stamp,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--profile",
        default="software/sim/config/vio_navigation_profile.json",
        help="JSON profile file with drift_validation defaults.",
    )
    ...
    parser.add_argument(
        "--max-time-offset-sec",
        type=float,
        help="Maximum allowed timestamp offset between VIO and ground-truth samples",
    )
    parser.add_argument(
        "--min-samples",
        type=int,
        help="Minimum matched sample count required for a valid pass/fail verdict",
    )
...
def main() -> int:
    ...
    vio_topic = str(
        _value_or_default(args.vio_topic, drift_config.get("vio_topic", "/scout1/openvins/odom"))
    )
    truth_topic = str(
        _value_or_default(
            args.ground_truth_topic,
            drift_config.get("ground_truth_topic", "/model/scout1/odometry"),
        )
    )
    duration_sec = float(_value_or_default(args.duration_sec, drift_config.get("duration_sec", 600)))
    max_rms_m = float(_value_or_default(args.max_rms_m, drift_config.get("max_rms_m", 3.0)))
    max_error_m = float(
        _value_or_default(args.max_error_m, drift_config.get("max_error_m", 8.0))
    )
    max_time_offset_sec = float(
        _value_or_default(
            args.max_time_offset_sec, drift_config.get("max_time_offset_sec", 0.25)
        )
    )
    min_samples = int(_value_or_default(args.min_samples, drift_config.get("min_samples", 100)))
    output_root = Path(
        _value_or_default(args.output_dir, drift_config.get("output_dir", "output/vio_navigation_drift"))
    )
    run_id = str(_value_or_default(args.run_id, drift_config.get("run_id", "latest")))
    ...
    pass_sample_count = sample_count >= min_samples
    passed = pass_sample_count and pass_rms and pass_max
    ...
    summary_lines = [
        ...
        f"sample_count={sample_count} (min {min_samples})",
        f"unmatched_sample_count={int(metrics.get('unmatched_sample_count', 0))}",
        f"max_time_offset_sec={max_time_offset_sec:.3f}",
